...
def test_secrets_client(monkeypatch):
    ...
    assert lambda_function.secrets_client() == boto3.client("secretsmanager", region_name="us-east-1")
...
def test_get_secret():
    ...
    assert lambda_function.get_secret("test") == {"username": "user", "password": "pass"}
