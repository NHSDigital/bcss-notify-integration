...
#       client = session.client(service_name='secretsmanager', region_name=REGION_NAME)
        ...
        if row > TS_THRESHOLD:
            print(f"Tablespace:{TABLESPACE_NAME} utilization is above threshold ({TS_THRESHOLD}%), Publishing message to SNS")
            response = sns_client.publish(TopicArn=SNS_ARN, Message=f"Tablespace:{TABLESPACE_NAME} utilization is above threshold. Please check immediately.")
            return {
                'statusCode': 200,
